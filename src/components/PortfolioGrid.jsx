...
export default function PortfolioGrid() {
  ...
  return (
    <section id="obras" className="py-24 bg-[#141210] relative">
            ...
            <button
              ...
              className={`px-5 py-2.5 rounded-full text-sm font-semibold transition-all duration-200 cursor-pointer ${
                activeCategory === cat.id
                  ? 'bg-gradient-to-r from-[#f59e0b] to-[#d97706] text-[#0a0908] shadow-lg shadow-amber-500/20 scale-105'
          ...
          {filteredWorks.map((work) => (
            ...
            >
              <div>
                ...
                {/* Title */}
                <h3 className="font-serif text-2xl font-bold text-[#f5f5f4] group-hover:text-[#f59e0b] transition-colors mb-2">
                  {work.title}
                </h3>
                ...
                <p className="text-sm text-[#a8a29e] line-clamp-3 mb-4 leading-relaxed">
                  {work.description}
                </p>
              </div>
              ...
              <div>
                {work.awards && work.awards.length > 0 && (
                  <div className="mb-4 p-2.5 rounded-lg bg-[#991b1b]/15 border border-[#991b1b]/30">
        ...
        <div className="modal-overlay" onClick={() => setSelectedWork(null)}>
          <div className="modal-content" onClick={(e) => e.stopPropagation()}>
            <button
              ...
              className="absolute top-4 right-4 text-[#a8a29e] hover:text-[#f5f5f4] p-2 focus:outline-none"
            >
              <X className="w-6 h-6" />
            </button>
            ...
            <h3 className="font-serif text-3xl font-bold text-[#f5f5f4] mb-2">
              {selectedWork.title}
            </h3>
            ...
            <div className="space-y-4 mb-6">
              <div>
                <h4 className="text-xs uppercase tracking-wider text-[#a8a29e] font-bold mb-1">Sinopsis</h4>
                <p className="text-sm text-[#d6d3d1] leading-relaxed">{selectedWork.description}</p>
              </div>

              {selectedWork.actedCharacter && (
                <div>
                  <h4 className="text-xs uppercase tracking-wider text-[#a8a29e] font-bold mb-1">Rol Interpretado</h4>
                  <p className="text-sm text-[#f59e0b] font-semibold">{selectedWork.actedCharacter}</p>
                </div>
              )}

              {selectedWork.details && (
                <div>
                  <h4 className="text-xs uppercase tracking-wider text-[#a8a29e] font-bold mb-1">Puesta en Escena y Contexto</h4>
                  <p className="text-sm text-[#a8a29e] leading-relaxed">{selectedWork.details}</p>
                </div>
              )}

              {selectedWork.awards && selectedWork.awards.length > 0 && (
                <div className="p-4 rounded-xl bg-[#991b1b]/20 border border-[#991b1b]/40">
                  <h4 className="text-xs uppercase tracking-wider text-[#f59e0b] font-bold mb-2 flex items-center gap-1.5">
                    <Award className="w-4 h-4" /> Reconocimientos y Distinciones
                  </h4>
                  <ul className="space-y-1 text-xs text-[#f5f5f4]">
                    {selectedWork.awards.map((award, i) => (
                      <li key={i} className="flex items-center gap-2">
                        <span className="w-1.5 h-1.5 rounded-full bg-[#f59e0b]" /> {award}
                      </li>
                    ))}
                  </ul>
                </div>
